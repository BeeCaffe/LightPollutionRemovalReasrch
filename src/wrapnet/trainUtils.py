...
def trainModel(model, warpping_net_model, valid_data, cam_path_dict, train_option):
    ...
    for i in range(train_option['epoch']):
        ...
        while iters < train_option.get('nums_train')//train_option['batch_size']:
            ...
            cam_train_batch = loadDataByPaths(cam_train_path, idx, size=train_option['train_size']).to(device)
            prj_train_batch = loadDataByPaths(prj_train_path, idx=idx, size=train_option['prj_size']).to(device)
            if DEBUG:
                img = cam_train_batch[0].permute(1, 2, 0).mul(255).to('cpu').detach().numpy()
                img = np.uint8(img)
                cv2.imshow('cam_train_batch ', img)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
            #get dark channel
            warpped_cam_train = warpping_net_model(cam_train_batch).detach()  # compensated prj input image x^{*}
            if DEBUG:
                img = warpped_cam_train[0].permute(1, 2, 0).mul(255).to('cpu').detach().numpy()
                img = np.uint8(img)
                cv2.imshow('warpped_cam_train ', img)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
            #predict and compute loss
            model.train()
            #gamma correct for masked_pred
            back_train = torch.pow(warpped_cam_train, train_option['backspect_gamma'])
            pro_train = torch.pow(warpped_cam_train, train_option['prospect_gamma'])
            pred = model(warpped_cam_train, pro_train, back_train)
            train_loss_batch, train_l2_loss_batch = computeLoss(pred, prj_train_batch, train_option.get('loss'))
            # perceptual loss
...
def evaluate(model, valid_data):
    ...
    with torch.no_grad():
        ...
        for i in range(0, num_valid // batch_size):
            ...
            # predict batch
            prj_valid_pred_batch = model(cam_valid_batch, cam_valid_batch, cam_valid_batch).detach()
