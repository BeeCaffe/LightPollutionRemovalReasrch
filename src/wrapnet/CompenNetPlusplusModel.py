...
class CompenNet(nn.Module):
    def __init__(self):
        ...
        self.conv7 = nn.Conv2d(3, 3 * 2 ** 2, kernel_size=3, padding=1)
    ...
    def forward(self, x, s):
        ...
        x = torch.clamp(self.prelu(self.pixel_shuffle(self.conv7(x))), max=1)
...
class WarpingNet(nn.Module):
    def __init__(self, grid_shape=(8, 8), out_size=(256, 256), with_refine=True):
    ...
    def simplify(self, x):
        ...
        tps_grid = F.grid_sample(coarse_affine_grid, coarse_tps_grid, mode='bilinear')
    ...
    def forward(self, x):
        ...
        # warp
        if DEBUG:
            img = x[0].permute(1, 2, 0).mul(255).to('cpu').numpy()
            img = np.uint8(img)
            cv2.imshow('ungrid sample', img)
            cv2.waitKey(0)
...
class CompenNetPlusplus(nn.Module):
    ...
    def forward(self, x, s):
        ...
        if DEBUG:
            img = x[0].permute(1, 2, 0).mul(255).to('cpu').detach().numpy()
            img = np.uint8(img)
            cv2.imshow('train img warpped', img)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
        if DEBUG:
            img = s[0].permute(1, 2, 0).mul(255).to('cpu').detach().numpy()
            img = np.uint8(img)
            cv2.imshow('surf img', img)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
