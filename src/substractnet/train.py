from src.substractnet.my_trainutils import *
import src.substractnet.valid as valid
import src.substractnet.substractnet_model as substract_model
import os
import torch
DEBUG = False
...
args = dict(dataset_root='datasets/dataset_512_no_gama_new/',
                ...
                loss_list=['l1+l2+ssim'], #l1, l2, ssim, hsvloss, vggloss, labloss, klloss
                loss='',
                model_name='substractnet',
                res_block_num_list=[1, 3, 6, 9, 12],
                res_block_num=[3],
                log_dir='log/',
...
desire_path = "input/substractnet/"
...
for num in args['res_block_num_list']:
    for gamma in args['gamma_list']:
        for weight in args['refloss_weight_list']:
            args['res_block_num'] = num
            args['gamma'] = gamma
            args['refloss_weight'] = weight
            #log file
            #load model
            reflection_model = substract_model.reflectionnet(3, 3, residel_num=args['res_block_num'])
            compensation_model = substract_model.compensationnet(3, 3, 3)
            if torch.cuda.device_count() >= 1: model_pro = nn.DataParallel(reflection_model, device_ids=device_ids).to(device)
            if torch.cuda.device_count() >= 1: model_pro = nn.DataParallel(compensation_model, device_ids=device_ids).to(device)
            for loss in args.get('loss_list'):
                ...
                reflection_pth, compensation_pth, valid_psnr, valid_rmse, valid_ssim, train_msg_1 = \
                    TrainProcess(reflection_model, compensation_model, warping_net_model, valid_data, cam_path_dict, args, weight).Train()
                ...
                if args['save_compensation']:
                    ...
                    reflection_save_dir = output_pth + 'reflection/'
                    compensation_save_dir = output_pth + 'compensation/'
                    if not os.path.exists(reflection_save_dir):
                        os.makedirs(reflection_save_dir)
                    if not os.path.exists(compensation_save_dir):
                        os.makedirs(compensation_save_dir)
                    valid.cmpImages(pth_path=reflection_pth, desire_test_path=desire_path, save_path=reflection_save_dir)
                    valid.cmpImages(pth_path=compensation_pth, desire_test_path=desire_path, save_path=compensation_save_dir)

                    print('Compensation images saved to {}', output_pth)
            # clear cache
            del reflection_model
            del compensation_model
            torch.cuda.empty_cache()
            print('----------------------------------------- Done! ------------------------------\n')
