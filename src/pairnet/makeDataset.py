...
args = {
    ...
    'camcap_path': 'H:\experiment data\\2019.12.10\camera-captured',#camera capture images path
    'grdtuth_path': 'H:\experiment data\\2019.12.10\ground-truth',#ground truth images path
    'cam_surf_path': 'data/dataset_512_gama1.5/cam/ref/surf.jpg',
    'dataset_root': 'data/dataset_512_no_gama_new',#dataset root directory
...
def buildDataset(camcap_path,grdtuth_path,dataset_root):
    ...
    ref_img = cv2.resize(cv2.imread(args['cam_surf_path'], cv2.IMREAD_UNCHANGED), (args['cols'], args['rows']))
    cv2.imwrite(join(cam_ref_dir, "surf.jpg"), ref_img)
    cv2.imwrite(join(cmp_ref_path, "surf.jpg"), ref_img)
