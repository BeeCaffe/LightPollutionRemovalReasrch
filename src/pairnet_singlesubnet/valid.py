...
def cmpImages(pth_path,mask_pth,desire_test_path,save_path):
    ...
    for i in range(len(os.listdir(desire_test_path))):
        desire_test = utils.readImgsMT(desire_test_path, index=[i], size=args.get('size')).to(device)
        with torch.no_grad():
            model.eval()
            mask = mask_net(desire_test)
            desire_test = torch.cat([desire_test, desire_test], dim=1)
            compen = model(desire_test, mask).detach()  # compensated prj input image x^{*}
